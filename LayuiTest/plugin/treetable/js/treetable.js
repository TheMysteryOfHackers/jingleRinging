layui.define([ "form" ], function(exports) {
	...
	tree.prototype.template = function(data) {
		...
		o.each(data, function(index, item) {
			var checked = t.c.is_checkbox && t.c.checked && o.inArray(item.id, t.c.checked) > -1 && 'checked', hide_class = 'class="' + (item.pid == 0 || item.pid == t.cache(item.pid) || t.c.is_open ? "" : "hide") + '"', tr = '<tr data-id="' + item.id + '" data-pid="' + item.pid + '" ' + hide_class + ">" + (t.c.is_checkbox ? '<td><div><input type="checkbox" lay-skin="primary" lay-filter="lay-t" ' + checked + '></div></td>' : "");
			// item.level = level[item.id] = item.pid > 0 ? (level[item.pid] +
			// 1) : 0;
			o.each(t.c.cols, function(idx, obj) {
				tr += '<td style="width:' + obj.width + '">';
				if (obj.field == t.c.field) {
					//tr += ("&nbsp;".repeat(level[item.id] * t.c.space));
					tr += ("&nbsp;".repeat(item.level * t.c.space));
	...
	tree.prototype.render = function(e) {
		...
		o(t.c.elem).html(template).on("click", "td", function() {
			var id = o(this).parents("tr").data("id"), pid = o(this).parents("tr").data("pid"), status = o(t.c.elem).find("tr[data-pid=" + id + "]").is(":visible"), dt = o(this).find("." + t.c.icon_class);
			if (dt.length) {
				if (status) {
					t.hide(id);
	...
	tree.prototype.parent_to_choose = function(id) {
		var t = this, pt = o(t.c.elem).find('[data-pid=' + id + ']'), pl = pt.find('[lay-skin=primary]:checked').length, bt = o(t.c.elem).find('[data-id=' + id + '] [lay-skin=primary]'), pid = o(t.c.elem).find('[data-id=' + id + ']').data('pid');
	...
	tree.prototype.cache = function(val, option) {
		var t = this, name = "tree-table-open-item", val = val.toString(), cache = t.get_cookie(name) ? t.get_cookie(name).split(",") : [], index = o.inArray(val, cache);
		if (option === undefined) {
