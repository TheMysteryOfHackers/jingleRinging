...
public abstract class CommonUtil extends CommonMethod {
	...
	public static int getLastDayOfMonth(String year, String month) {
		Calendar calendar = Calendar.getInstance();
		calendar.set(Calendar.YEAR, int2(year));
		calendar.set(Calendar.MONTH, int2(month) - 1);
		int minday = calendar.getActualMinimum(Calendar.DAY_OF_MONTH);
		return calendar.getActualMaximum(Calendar.DATE);
	}

	public static void main(String[] args) {
		// CommonUtil.isBlank(str) 判断是否为空
		// CommonUtil.isEquals(dec1, dec2) 判断是否相等
		// System.out.println(CommonUtil.date());
		// System.out.println(CommonUtil.date1());
		// System.out.println(CommonUtil.date2());
		// // 根据字符串转date
		// Date date = CommonUtil.getDate("20180921", "ISO0");

		// CommonUtil.compareTo(dec1, dec2)

		// CommonUtil.abs(input)
		System.out.println(CommonUtil.evalr(3, 4));
	}
